...
| 接口 `interface` | ⚠️ 部分 | 语法支持，运行时待完善 |
| 抽象类 `abstract` | ⚠️ 部分 | 语法支持，运行时待完善 |
| 方法重载 | ❌ 未完成 | |
