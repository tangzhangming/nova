...
Sola 标准库提供的 PostgreSQL 数据库客户端，支持连接池、查询、事务等功能。
...
// 执行
$affected := $client->execute("INSERT INTO users (name) VALUES ($1)", ["Alice"]);
...
```sola
public function query(string $sql, string[] $params = []): PgResult
```

**参数：**
- `$sql`: SQL 语句，支持 `$1, $2, ...` 占位符
- `$params`: 参数数组
...
**示例：**
```sola
$result := $client->query("SELECT * FROM users WHERE age > $1", ["18"]);
$rows := $result->all();
```
...
**示例：**
```sola
$affected := $client->execute("UPDATE users SET name = $1 WHERE id = $2", ["Alice", "1"]);
```
...
**参数：**
- `$callback`: 事务回调函数，接收 `PgConnection` 参数
...
$client := new PgClient("127.0.0.1", 5432, "root", "password", "mydb", $config);
```
...
```sola
use sola.database.postgresql.{PgPool, PoolConfig};

$config := new PoolConfig();
$pool := new PgPool($config, "127.0.0.1", 5432, "root", "password", "mydb");
...
1. **占位符格式**：PostgreSQL 使用 `$1, $2, ...` 作为占位符，而不是 `?`
2. **参数索引**：参数索引从 1 开始（`$1` 对应 `$params[0]`）
3. **事务**：使用 `transaction()` 方法可以自动处理事务的提交和回滚
