...
// 可配置的栈大小限制
var (
	// stackMaxConfig 操作数栈的最大深度（默认 1024）
	stackMaxConfig = 1024

	// framesMaxConfig 调用栈的最大深度（默认 1024）
	framesMaxConfig = 1024
)

const (
	// StackMax 定义操作数栈的最大深度（4096 个槽位）。
	//
	// 操作数栈用于存储：
	//   - 表达式计算的中间结果
	//   - 函数调用的参数
	...
	//   - 临时对象引用
	//
	// BUG FIX 2026-01-10: 栈大小翻倍 - 允许更深的调用层次和更复杂的计算
	StackMax = 4096

	// FramesMax 定义调用栈的最大深度（1024 层）。
	//
	...
	//   - 栈基址 (BaseSlot)
	//
	// BUG FIX 2026-01-10: 增大调用栈上限 - 修复递归深度限制过小的问题
	FramesMax = 1024
)
...
func (vm *VM) execute() InterpretResult {
	...
	// 注意: 指令数上限已移除
	// ------------------------------------------------------------------------
	// 之前有 5 亿条指令上限，但这会限制大规模计算（如 fib(35) 等）。
	// 如需防止死循环，建议在外部使用超时机制。
